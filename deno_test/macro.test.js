...
Deno.test("Twig.js Macro ->", async t => {
    ...
    await t.step("it should run macro with self reference twice", async () => {
        ...
        const testTemplate = await twig.twig({
            ref: "import-macro-self-twice"
        });
        assertEquals(
            testTemplate.render({}).trim(),
            '<p><input type="text" name="username" value="" size="20" /></p><p><input type="text" name="password" value="" size="20" /></p>'
        );
    });
    ...
    await t.step(
        ...
        async () => {
            ...
            // Load the template
            const testTemplate = await twig.twig({
                ref: "import-wrapped-macro-self"
            });
            assertEquals(
                testTemplate.render({}).trim(),
                '<p><div class="field"><input type="text" name="username" value="" size="20" /></div></p>'
            );
        }
    );
    ...
    await t.step(
        ...
        async () => {
            ...
            // Load the template
            const testTemplate = await twig.twig({
                ref: "import-macro-context-self"
            });
            assertEquals(
                testTemplate.render({ greetings: "Howdy" }).trim(),
                "Howdy Twigjs"
            );
        }
    );
    ...
    await t.step(
        "it should run wrapped macro with default value for a parameter and self reference",
        async () => {
            await new Promise((res, rej) => {
                twig.twig({
            ...
            // Load the template
            const testTemplate = await twig.twig({
                ref: "import-macro-defaults-self"
            });
    ...
    await t.step("it should run wrapped macro inside blocks", async () => {
        ...
        // Load the template
        const testTemplate = await twig.twig({
            ref: "import-macro-inside-block"
        });
        assertEquals(
            testTemplate.render({}).trim(),
            'Welcome <div class="name">Twig Js</div>'
        );
    });
    ...
    await t.step("should support inline includes by ID", async () => {
        ...
        const template = await twig.twig({
            ...
            data:
                'template with {% from "hello" import echo %}{{ echo("Twig.js") }}'
        });
